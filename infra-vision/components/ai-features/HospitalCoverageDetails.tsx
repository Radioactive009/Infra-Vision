...
export function HospitalCoverageDetails({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    fetch("/api/hospital-coverage")
      ...
      .then(setData)
      .catch((err) => {
        console.error("Failed to load hospital coverage:", err);
      })
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-100 via-blue-50 to-teal-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading hospital coverage data...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-100 via-blue-50 to-teal-50 py-10 px-4">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="rounded-3xl bg-white shadow-lg p-8 space-y-6 border border-gray-100">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Hospital Coverage Analysis</h1>
            <p className="text-gray-500">Live model results from city-wide healthcare coverage predictions</p>
          </div>
            ...
            <table className="w-full text-sm">
              <thead className="text-gray-500 border-b-2 border-gray-200">
                <tr>
                  ...
                  <th className="py-3 text-left font-semibold">Facilities</th>
              ...
              <tbody>
                {data.zones.map((z, index) => (
                  ...
                  >
                    ...
                    <td className="py-4 text-gray-700">{z.num_facilities}</td>
                    <td className="py-4 text-gray-700">{z.total_capacity > 0 ? z.total_capacity.toLocaleString() : 'N/A'}</td>
                    <td className="py-4">
